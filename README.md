...
> 🐣 **如果你完全不懂 Linux、是第一次接触命令行**：先别看下面的，直接去 👉 [🐣 安装入门指南（写给纯小白）](./INSTALL_GUIDE.md) —— 那篇指南假设你零基础，从「怎么把镜像写入 U 盘」开始手把手教。
...
把输出的哈希粘贴到 `vars/default.nix` 的 `initialHashedPassword` 处（替换掉 `null`）。

> 如果你跳过这步，装好后用户 `feng` 没有密码，greetd 登录界面进不去。
...
参考单系统"第二步"的说明，先用 `mkpasswd -m yescrypt` 生成密码哈希并写入 `vars/default.nix`。
...
装系统前先用 `mkpasswd -m yescrypt` 生成密码哈希，粘贴到 `vars/default.nix` 的 `initialHashedPassword` 处。详情见台式机章节的说明。
